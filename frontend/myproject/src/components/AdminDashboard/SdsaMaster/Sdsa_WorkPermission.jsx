...
const Sdsa_WorkPermission = () => {
  ...
  useEffect(() => {
    const fetchSdsaInfo = async () => {
      ...
      try {
        ...
        // Get existing work permissions from SDSA user
      ...
      try {
        ...
        if (workIconsData.length > 0) {
          ...
          workIconsData.forEach(icon => {
            // Check if this work icon is already in SDSA user's permissions
  ...
  const savePermissions = async () => {
    if (!userId) {
      alert('Please select a SDSA user first');
      return;
    }
    ...
    try {
      ...
      console.log('Updating SDSA user with data:', updateData);

      // Use PATCH request for SDSA user update
      await api.patch(`/sdsa-users/${userId}/`, updateData);
      ...
      console.error('Error saving permissions to SDSA user:', err);
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div>
          <h1 style={{ 
            ...
            SDSA Work Permission Management
          </h1>
          {sdsaInfo && (
      ...
      <div style={{ 
        ...
        >
          {saving ? 'Saving...' : `Save ${selectedCount} Permission(s) for ${getSdsaDisplayName()}`}
        </button>
      </div>
...
export default Sdsa_WorkPermission;